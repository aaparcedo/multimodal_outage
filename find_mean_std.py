...
dataset = BlackMarbleDataset(dir_image, size='M', case_study=entire_set)
...
for data in data_loader:
    past_tensor, future_tensor = data
    ...
    mean += data_tensor.mean(dim=(0, 2, 3), keepdim=True)
    std += data_tensor.std(dim=(0, 2, 3), keepdim=True)
